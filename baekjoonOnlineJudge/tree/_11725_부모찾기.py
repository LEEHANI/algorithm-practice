...
방법1은 메모리를 낭비하니, 방법2로 선택하자.

예제 1번의 연결 정보를 인접 리스트에 저장해보자.
[[], [4, 6], [4], [6, 5], [1, 2, 7], [3], [1, 3], [4]]
루트 노드가 1번이므로 1번 노드부터 보자. 노드 1번이랑 연결되어있는 노드는 4, 6번이다. 루트 노드이므로 4, 6은 자식노드이다. visited에 4, 6번 노드에 부모 노드를 저장해놓는다. [-1, 0, -1, -1, 1, -1, 1, -1]
그 다음 1번의 첫번째 자식인 4번을 부모노드로 놓는다(Queue). 자식노드 [1, 2, 7]에서 부모노드가 없는 자식을 찾아 부모 visited에 부모노드 4를 저장해놓는다. [-1, 0, 4, -1, 1, -1, 1, 4]
큐에 데이터가 없을 때 까지 반복한다. 
