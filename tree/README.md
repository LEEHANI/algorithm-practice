...
- ```전위순회```(preorder), ```중위순회```(inorder), ```후위순회```(postorder) 탐색이 가능하다 


### 전위순회, 중위순회, 후위순회 방법
![example](./backjoon_problem_1991_example.png)
...
5. 자식 인덱스를 저장하는 방법: 부모노드가 i라면 A[i][0]에는 왼쪽 자식을, A[i][1]에는 오른쪽 자식을 저장  
